...
class Dangnhap extends CI_Controller {
    ...
    public function dangnhap(){
        ...
        if($this->form_validation->run() ==TRUE){
            ...
            $this->data['title']='Beptot - Đăng nhập tài khoản';
            $this->data['view']='dangnhap';
            $this->load->view('frontend/layout',$this->data);
        }     
    }
    ...
    public function dangky(){
        ...
        if($this->form_validation->run() ==TRUE){
            ...
            $config['smtp_user']    = 'beptot@gmail.com';
            ...
            $this->email->from('beptot@gmail.com', 'Bếp Tốt');
            $this->email->to($email);
            $this->email->subject('Hệ thống Bếp Tốt - Quà thành viên mới');
            $this->email->message('Bạn đã trở thành thành viên mới của cửa hàng Bếp Tốt, Cửa hàng tặng bạn 1 mã giảm giá giảm 100.000 đ : '.$tempcoupon.' , Mã này có giá trị tới ngày '.$tempdatelimit.'
        ...
        $this->data['title']='Bếp Tốt - Đăng ký tài khoản';   
    ...
    public function forget_password(){
        ...
        if($this->form_validation->run() ==TRUE){
            ...
            $config['smtp_port']    = '465';
            $config['smtp_timeout'] = '7';
            $config['smtp_user']    = 'edubook.store@gmail.com';
            $config['smtp_pass']    = 'cqfmfmrtudhcmahw';
            $config['charset']    = 'utf-8';
            $config['newline']    = "\r\n";
            $config['wordwrap'] = TRUE;
            ...
            $config['validation'] = TRUE;   
            $this->email->initialize($config);
            $this->email->from('edubook.store@gmail.com', 'Bếp Tốt');
            $this->email->to($list['email']);
            $this->email->subject('Bếp Tốt - Lấy lại mật khẩu');
        ...
        $this->data['title']='Bếp tốt - Quên mật khẩu';   
    ...
    public function reset_password_new($id){
        ...
        if($this->form_validation->run() ==TRUE){ 
           ...
           if($this->Mcustomer->customer_check_id_email($id, $email)!=FALSE){
            ...
            $this->data['title']='Bếp tốt - Cập nhật mật khẩu mới';
